...
func inspectArchiveOrg(ctx context.Context, input string) (*InspectResult, error) {
	...
	if req, err := http.NewRequestWithContext(ctx, http.MethodGet, torrentURL, nil); err == nil {
		req.Header.Set("User-Agent", "Digwire/0.4.0 (Swarm Inspector)")
	...
	req.Header.Set("User-Agent", "Digwire/0.4.0 (Metadata Inspector)")
...
func inspectHTTPFile(ctx context.Context, fileURL string) (*InspectResult, error) {
	...
	req.Header.Set("User-Agent", "Digwire/0.4.0 (HTTP Inspector)")
	...
	if err == nil {
		headReq.Header.Set("User-Agent", "Digwire/0.4.0 (HTTP Inspector)")
