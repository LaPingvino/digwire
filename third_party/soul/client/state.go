...
func (s *State) Download(ctx context.Context, file Download) (status chan string, e chan error) {
	...
	go func() {
		...
		sendStatus("sending queue message")
	...
	waitTransfer:
		for {
			...
			case <-p.initListeners.uploadFailed:
				...
				return
			case q := <-p.initListeners.placeInQueueResponse:
				sendStatus(fmt.Sprintf("queued at position %d", q.Place))
			case t := <-p.initListeners.transferRequest:
				transfer = t
				break waitTransfer
			}
		}

		sendStatus(transfer.Direction.String())
		...
		sendStatus("response message sent")
		...
		go func() {
			...
			case <-ctx.Done():
				fileConn.Close()
			case <-p.initListeners.uploadDenied:
				fileConn.Close()
			case <-p.initListeners.uploadFailed:
				fileConn.Close()
			}
		...
		sendStatus("file connection established")
