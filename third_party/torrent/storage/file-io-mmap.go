...
func (me *fileMmap) close() (err error) {
	...
	me.closed = true
	var unmapErr error
	// Zero-length files are never mapped (mmap rejects a 0 length with EINVAL).
	if me.m != nil {
		unmapErr = me.m.Unmap()
	}
	return errors.Join(unmapErr, me.f.Close())
}
...
// mapFile maps the whole file, leaving zero-length files unmapped since mmap rejects a 0 length
// with EINVAL. A nil MMap behaves as an empty mapping for reads and writes go through the file.
func mapFile(f *os.File, prot int) (mm mmap.MMap, err error) {
	fi, err := f.Stat()
	if err != nil {
		return
	}
